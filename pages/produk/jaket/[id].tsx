...
import { JAKET } from "@/data/produk/jaket";
...
export default function JaketDetailPage({ product }: ProductPageProps) {
  return (
    <>
      <NextSeo
      ...
      <Box pt={{ base: 8, md: 10 }} pb={{ base: 6, md: 8 }}>
        <Container maxW="container.xl" px={{ base: 5, lg: 10 }}>
          <Link href="/produk/jaket" passHref legacyBehavior>
            <Button
              as="a"
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  const product = JAKET.find((p) => p.id === params?.id);
