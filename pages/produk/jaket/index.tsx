...
const filterOptions: FilterOption[] = [
  ...
  ...JAKET_TYPES.map((type) => ({ label: type, value: kebabCase(type) })),
];
      ...
      >
        {filteredItems.map((item) => (
          <GridItem key={item.id} w="100%">
            <ProductCard product={item} category="jaket" />
          </GridItem>
        ))}
      </Grid>
    </>
...
export default function JaketPage() {
  ...
  return (
    <>
      <NextSeo
        title="Sleep Walker Produk | Jaket"
        description="Temukan koleksi jaket Sleep Walker dengan berbagai pilihan."
      />
