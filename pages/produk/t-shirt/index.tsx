...
import { TSHIRT, TSHIRT_TYPES } from "@/data/produk/tshirt";
...
const filterOptions: FilterOption[] = [
  ...
  ...TSHIRT_TYPES.map((type) => ({ label: type, value: kebabCase(type) })),
];
      ...
      >
        {filteredItems.map((item) => (
          <GridItem key={item.id} w="100%">
            <ProductCard product={item} category="t-shirt" />
          </GridItem>
        ))}
      </Grid>
    </>
...
export default function TShirtPage() {
  ...
  return (
    <>
      <NextSeo
        title="Sleep Walker Produk | T-Shirt"
        description="Temukan koleksi t-shirt Sleep Walker dengan berbagai pilihan."
      />
