...
import { JERSEY, JERSEY_TYPES } from "@/data/produk/jersey";
...
const filterOptions: FilterOption[] = [
  ...
  ...JERSEY_TYPES.map((type) => ({ label: type, value: kebabCase(type) })),
];
      ...
      >
        {filteredItems.map((item) => (
          <GridItem key={item.id} w="100%">
            <ProductCard product={item} category="jersey" />
          </GridItem>
        ))}
      </Grid>
    </>
...
export default function JerseyDanClothing() {
  ...
  return (
    <>
      <NextSeo
        title="Sleep Walker Produk | Jersey"
        description="Koleksi jersey futsal dan racing Sleep Walker berkualitas tinggi dengan desain custom dan ready stock."
      />
